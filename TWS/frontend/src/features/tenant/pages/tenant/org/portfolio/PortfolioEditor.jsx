    ...
    <div className="min-h-full bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur dark:border-slate-800 dark:bg-slate-900/95">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          ...
          <div className="hidden items-center gap-3 md:flex"><span className="text-xs text-slate-500 dark:text-slate-400">Story completeness</span><div className="h-2 w-32 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-full bg-violet-500" style={{ width: `${completion}%` }} /></div><span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{completion}%</span></div>
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={publish} disabled={!canWrite} className={`rounded-lg px-3.5 py-2 text-sm font-semibold text-white disabled:opacity-40 ${item.status === 'published' ? 'bg-slate-700 hover:bg-slate-800' : 'bg-violet-600 hover:bg-violet-700'}`}>{item.status === 'published' ? 'Return to draft' : 'Publish internally'}</button>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 p-5 md:p-8 lg:grid-cols-[minmax(0,1fr)_310px]">
        <main className="space-y-6">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-7">
            <div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-violet-600 dark:text-violet-400">The snapshot</p><h1 className="mt-1 text-2xl font-bold text-slate-950 dark:text-white">Make the value obvious</h1></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold capitalize text-slate-600 dark:bg-slate-800 dark:text-slate-300">{item.status}</span></div>
            <div className="space-y-5">
              ...
              <label className="flex items-center gap-3 text-sm font-medium text-slate-700 dark:text-slate-300"><input type="checkbox" checked={Boolean(item.client?.confidential)} onChange={e => updateNested('client', 'confidential', e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-violet-600 dark:border-slate-700 dark:bg-slate-950" /> Client identity is confidential / NDA-protected</label>
            </div>
          </section>

          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-7">
            <p className="text-xs font-bold uppercase tracking-widest text-violet-600 dark:text-violet-400">The story</p><h2 className="mb-6 mt-1 text-xl font-bold text-slate-950 dark:text-white">Context → decisions → impact</h2>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-7">
            <div className="mb-5 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-violet-600 dark:text-violet-400">Proof</p><h2 className="mt-1 text-xl font-bold text-slate-950 dark:text-white">Measurable results</h2></div><button type="button" onClick={addMetric} className="flex items-center gap-1.5 rounded-lg border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-700 hover:bg-violet-50 dark:border-violet-500/40 dark:text-violet-300 dark:hover:bg-violet-500/10"><PlusIcon className="h-4 w-4" /> Metric</button></div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-7">
            <p className="text-xs font-bold uppercase tracking-widest text-violet-600 dark:text-violet-400">Media</p><h2 className="mb-2 mt-1 text-xl font-bold text-slate-950 dark:text-white">Show the work in context</h2><p className="mb-5 text-sm text-slate-500 dark:text-slate-400">Images up to 5 MB, documents up to 25 MB, or MP4/WebM/MOV video up to 100 MB.</p>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {(item.assets || []).map(asset => <div key={asset._id} className={`relative overflow-hidden rounded-xl border-2 ${String(item.coverAssetId) === String(asset._id) ? 'border-violet-500' : 'border-transparent bg-slate-100 dark:bg-slate-800'}`}>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 md:p-7">
            <p className="text-xs font-bold uppercase tracking-widest text-violet-600 dark:text-violet-400">Story builder</p>
            ...
            <div className="space-y-3">
              {(item.blocks || []).map((block, index) => <div key={block._id || `${block.type}-${index}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-950">
                <div className="mb-3 flex items-center gap-2">
                  <span className="rounded-md bg-violet-100 px-2 py-1 text-xs font-bold uppercase text-violet-700 dark:bg-violet-500/15 dark:text-violet-300">{block.type}</span>
                ...
                {block.type === 'embed' ? <p className="truncate text-sm text-slate-600 dark:text-slate-300">{block.embed?.url}</p> : block.type === 'divider' ? <hr className="my-3 border-slate-300 dark:border-slate-700" /> : ['image', 'video', 'gallery', 'document'].includes(block.type) ? <div>
                  ...
                  <div className="flex flex-wrap gap-2">{(item.assets || []).filter(asset => block.type === 'gallery' || block.type === 'image' ? asset.kind === 'image' : block.type === 'video' ? asset.kind === 'video' : asset.kind === 'document').map(asset => {
                    ...
                    return <button type="button" key={asset._id} onClick={() => updateBlock(index, { assetIds: selected ? block.assetIds.filter(value => String(value) !== String(asset._id)) : [...(block.assetIds || []), asset._id] })} className={`rounded-lg border px-2.5 py-1.5 text-xs ${selected ? 'border-violet-500 bg-violet-50 text-violet-700 dark:bg-violet-500/10 dark:text-violet-300' : 'border-slate-200 dark:border-slate-700'}`}>{asset.originalName}</button>;
                  })}</div>
